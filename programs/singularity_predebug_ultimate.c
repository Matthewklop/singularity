 ...
 * Profile: ultimate
 * unroll=8 prefetch=2 align=1 hints=1
 * pmc=1 lbr=1 pt=1 batch_msr=1
...
static inline void pause_loop(int n) {
    /* Fully unrolled pause loop */
    for (int i = 0; i < n; i += 8) {
        asm volatile("pause;pause;pause;pause;pause;pause;pause;pause" : : : "memory");
    }
}
...
static void read_msr_batch(uint32_t *addrs, uint64_t *vals, int n) {
    for (int i = 0; i < n; i++) {
        if (msr_fd >= 0) pread(msr_fd, &vals[i], 8, addrs[i]);
    }
}
...
int main() {
    printf("SINGULARITY PREDEBUG v2 [ultimate]\n");
    init_msr();
